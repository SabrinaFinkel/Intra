 ...
 */
 public class OverrideSocksHandler extends UdpOverrideSocksHandler {
  ...
  public void doConnect(Session session, CommandMessage commandMessage) throws IOException {
    ...
    try {
      listener = runPipes(session, socket);
      if (remoteServerPort == HTTPS_PORT && listener.downloadBytes == 0 && listener.uploadBytes > 0
        ...
        listener = splitRetry(socket.getRemoteSocketAddress(), session, listener);
      }
  ...
  /**
   * This function starts pipes for upload and download and blocks until they stop.
   * @param session An initiated session to the client
   * @param socket A connected socket to the destination
   * @throws IOException if there is a network error
   * @throws InterruptedException if this thread is interrupted.
   * @return A StatsListener containing final stats on the socket, which has now been closed.
   */
  private static StatsListener runPipes(final Session session, final Socket socket)
      throws IOException, InterruptedException {
    final OutputStream sessionDownload = session.getOutputStream();
    final InputStream sessionUpload = session.getInputStream();
    final InputStream socketDownload = socket.getInputStream();
    final OutputStream socketUpload = socket.getOutputStream();

    StreamPipe upload = new StreamPipe(sessionUpload, socketUpload, UPLOAD);
    ...
    StatsListener listener = new StatsListener(socket.getPort());
   ...
   * fails.
   * @param dest A server to which a connection has previously failed after the first segment.
   * @param session A connected client which has sent data but not received any.
   ...
   * @throws InterruptedException if this thread is interrupted.
   */
  private StatsListener splitRetry(SocketAddress dest, Session session,
    ...
    int limit = MIN_SPLIT + RANDOM.nextInt() % (MAX_SPLIT + 1 - MIN_SPLIT);
    ...
    try {
      ...
      StatsListener newListener = runPipes(session, socket);
